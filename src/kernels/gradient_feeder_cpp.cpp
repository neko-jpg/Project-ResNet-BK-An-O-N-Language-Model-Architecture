// gradient_feeder_cpp.cpp
// C++ extension for fast gradient scaling (No CUDA compiler required)
// Uses PyTorch's existing CUDA-optimized tensor operations
// This avoids CUDA version mismatch issues since no custom CUDA code is compiled
...
// Python bindings
PYBIND11_MODULE(gradient_feeder_cpp, m) {
    m.doc() = "C++ extension for gradient feeding (CUDA version independent)";
    
    m.def("scale_all_gradients", &scale_all_gradients, 
          "Scale all gradients");
