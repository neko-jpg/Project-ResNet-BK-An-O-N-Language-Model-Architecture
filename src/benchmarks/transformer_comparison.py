"""
Side-by-side comparison between ResNet-BK and a standard Transformer.

This script trains both models on the same dataset (WikiText-103 or C4 subset)
with identical hyperparameters so we can measure:
    * Perplexity progression
    * Training throughput (tokens / second)
    * Validation perplexity at fixed intervals
    * Maximum sequence length support

Usage:
    python -m src.benchmarks.transformer_comparison --dataset wikitext103 --total_steps 500
"""
...
class ComparisonConfig:
    dataset: str = "wikitext103"  # {"wikitext103", "c4"}
    seq_len: int = 2048
    batch_size: int = 8
    ...
    grad_clip: float = 1.0
    data_limit: Optional[int] = 5_000_000  # tokens
    val_limit: Optional[int] = 500_000
    save_dir: str = "benchmarks/results"
    device: str = "auto"  # "auto", "cuda", or "cpu"

    # Shared model dimensions so both architectures are comparable.
    d_model: int = 256
    n_layers: int = 8
    num_heads: int = 8
    ffn_dim: int = 1024
    ...
    for line in texts:
        for token in line.strip().split():
            ...
            if limit_tokens is not None and total_tokens >= limit_tokens:
                tensor = torch.tensor(ids[:limit_tokens], dtype=torch.long)
                return tensor
...
def load_language_dataset(
    ...
    if name == "wikitext103":
        ...
        train_split = load_dataset("c4", "en", split="train[:0.1%]")
        val_split = load_dataset("c4", "en", split="validation[:0.1%]")
...
def run_comparison(cfg: ComparisonConfig):
    ...
    output_path = Path(cfg.save_dir) / f"{cfg.dataset}_resnetbk_vs_transformer.json"
    payload = {
        "config": asdict(cfg),
        "vocab_size": vocab["size"],
        "results": {
            "resnet_bk": resnet_results,
...
def build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="ResNet-BK vs Transformer benchmark")
    parser.add_argument("--dataset", type=str, default="wikitext103", choices=["wikitext103", "c4"])
    ...
    parser.add_argument("--data_limit", type=int, default=5_000_000)
    parser.add_argument("--val_limit", type=int, default=500_000)
    parser.add_argument("--save_dir", type=str, default="benchmarks/results")
