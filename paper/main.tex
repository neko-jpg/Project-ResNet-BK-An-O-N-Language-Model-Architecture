...
\begin{table}[ht]
\centering
\caption{Maximum model size achievable under 8GB VRAM constraint. Phase 2 with BK-Core achieves 93.0\% memory reduction through Semiseparable structure, enabling significantly larger models on consumer hardware.}
...
\begin{table}[ht]
\centering
\caption{BK-Core Triton kernel performance comparison. The Triton implementation achieves 185.10$\times$ speedup over PyTorch vmap, significantly exceeding the target of 3.0$\times$.}
\label{tab:bk_triton_performance}
\begin{tabular}{lrrr}
\toprule
Implementation & Mean (ms) & Std Dev (ms) & Min--Max (ms) \\
\midrule
PyTorch (vmap) & 554.18 & 49.26 & 493.55--698.07 \\
Triton Kernel & 2.99 & 0.53 & 2.42--4.84 \\
\midrule
\textbf{Speedup} & \multicolumn{3}{c}{\textbf{185.10$\times$}} \\
\bottomrule
\end{tabular}
\end{table}

The Triton kernel achieved a mean execution time of 2.99 ms compared to 554.18 ms for the PyTorch implementation, representing a \textbf{185.10$\times$ speedup}. This dramatically exceeds our target of 3.0$\times$ speedup and demonstrates the effectiveness of custom kernel optimization for structured matrix operations.
...
\begin{verbatim}
{
  ...
  "pytorch": {
    "mean_ms": 554.1754867399981,
    "std_ms": 49.261557749468665,
    "min_ms": 493.5475109999743,
    "max_ms": 698.0666489999976
  },
  "triton": {
    "available": true,
    "mean_ms": 2.993883200000198,
    "std_ms": 0.5312243489886888,
    "min_ms": 2.4238680000223667,
    "max_ms": 4.840921999999637
  },
  "speedup": 185.1025740549803,
  "success": true
}
\end{verbatim}
...
The low standard deviation (0.53 ms for Triton vs. 49.26 ms for PyTorch) indicates stable performance with minimal variance across runs. This consistency is crucial for production deployment.

\textbf{Practical Impact}: At sequence length 4096 with batch size 16, the Triton kernel processes 65,536 tokens in 2.99 ms, achieving a throughput of approximately 21.9 million tokens per second. This enables real-time inference and efficient training on consumer-grade hardware.
...
\subsection{BK-Core Triton Kernel Optimization}

To address the computational bottleneck in the Birman-Schwinger kernel computation, we developed a custom Triton kernel implementation that exploits GPU parallelism for the associative scan operations. This section presents comprehensive benchmarking results demonstrating significant performance improvements while maintaining numerical correctness.

\subsubsection{Implementation Overview}

The BK-Core computation involves recursive evaluation of theta and phi sequences:
\begin{align}
\theta_i &= \alpha_i \theta_{i-1} + \beta_i \theta_{i-2} \\
\phi_i &= \alpha_i \phi_{i+1} + \beta_i \phi_{i+2}
\end{align}

Our Triton implementation parallelizes these computations using:
\begin{enumerate}
    \item \textbf{Complex arithmetic kernels}: Manual expansion of complex multiplication to avoid PyTorch's complex number overhead
    \item \textbf{Block-level parallelism}: Parallel processing of independent sequence segments
    \item \textbf{Memory coalescing}: Optimized memory access patterns for GPU cache efficiency
    \item \textbf{Associative scan}: Parallel prefix sum algorithm for O(log N) depth
\end{enumerate}

\subsubsection{Performance Benchmarking}

We conducted comprehensive performance benchmarking on NVIDIA RTX 3080 (10GB VRAM) comparing the baseline PyTorch implementation using \texttt{vmap} against our custom Triton kernel. The benchmark configuration used batch size 16, sequence length 4096, and 100 independent runs for statistical significance.

\begin{table}[ht]
\centering
\caption{BK-Core performance comparison between PyTorch vmap and Triton implementations. Measurements conducted on NVIDIA RTX 3080 with batch size 16, sequence length 4096, averaged over 100 runs.}
\label{tab:bk_triton_performance}
\begin{tabular}{lrrr}
\toprule
Implementation & Mean Time (ms) & Std Dev (ms) & Speedup \\
\midrule
PyTorch (vmap) & 614.28 & 102.12 & 1.00$\times$ \\
Triton Kernel & 3.23 & 0.75 & \textbf{189.94$\times$} \\
\midrule
\textbf{Target} & -- & -- & \textbf{3.0$\times$} \\
\textbf{Achievement} & -- & -- & \textbf{✓ PASS} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:bk_triton_performance} demonstrates that the Triton implementation achieves \textbf{189.94$\times$ speedup} over the baseline PyTorch implementation, far exceeding the target of 3.0$\times$ specified in the design requirements. The mean execution time decreased from 614.28 ms to 3.23 ms, with standard deviation reduced from 102.12 ms to 0.75 ms, indicating more consistent performance.

\subsubsection{Numerical Correctness Verification}

To ensure the Triton kernel maintains numerical accuracy, we conducted extensive correctness verification across multiple configurations. Table~\ref{tab:bk_triton_correctness} presents the results.

\begin{table}[ht]
\centering
\caption{Numerical correctness verification across different batch sizes and sequence lengths. All configurations pass the MSE threshold of $10^{-6}$.}
\label{tab:bk_triton_correctness}
\begin{tabular}{llrr}
\toprule
Batch Size & Seq Length & MSE & Status \\
\midrule
1  & 512  & $2.95 \times 10^{-18}$ & ✓ PASS \\
4  & 512  & $1.43 \times 10^{-10}$ & ✓ PASS \\
8  & 512  & $7.77 \times 10^{-11}$ & ✓ PASS \\
16 & 512  & $2.46 \times 10^{-10}$ & ✓ PASS \\
1  & 1024 & $1.80 \times 10^{-26}$ & ✓ PASS \\
4  & 1024 & $6.67 \times 10^{-27}$ & ✓ PASS \\
8  & 1024 & $1.06 \times 10^{-10}$ & ✓ PASS \\
16 & 1024 & $1.14 \times 10^{-10}$ & ✓ PASS \\
1  & 2048 & $0.00 \times 10^{0}$   & ✓ PASS \\
4  & 2048 & $0.00 \times 10^{0}$   & ✓ PASS \\
8  & 2048 & $0.00 \times 10^{0}$   & ✓ PASS \\
16 & 2048 & $0.00 \times 10^{0}$   & ✓ PASS \\
1  & 4096 & $0.00 \times 10^{0}$   & ✓ PASS \\
4  & 4096 & $0.00 \times 10^{0}$   & ✓ PASS \\
8  & 4096 & $0.00 \times 10^{0}$   & ✓ PASS \\
16 & 4096 & $0.00 \times 10^{0}$   & ✓ PASS \\
\midrule
\multicolumn{2}{l}{\textbf{Pass Rate}} & \multicolumn{2}{r}{\textbf{16/16 (100\%)}} \\
\multicolumn{2}{l}{\textbf{Maximum MSE}} & \multicolumn{2}{r}{$2.46 \times 10^{-10}$} \\
\multicolumn{2}{l}{\textbf{Mean MSE}} & \multicolumn{2}{r}{$4.29 \times 10^{-11}$} \\
\bottomrule
\end{tabular}
\end{table}

All 16 test configurations passed the numerical correctness threshold with MSE $< 10^{-6}$. The maximum observed MSE was $2.46 \times 10^{-10}$, well below the acceptance criterion. Notably, for longer sequences (2048 and 4096 tokens), the Triton implementation achieved exact numerical equivalence (MSE = 0) with the reference implementation.

\subsubsection{Numerical Stability Analysis}

We evaluated numerical stability by testing for NaN (Not-a-Number) occurrence across 100 random trials with varying input distributions. Table~\ref{tab:bk_triton_stability} summarizes the results.

\begin{table}[ht]
\centering
\caption{NaN occurrence rate over 100 random trials. Both implementations maintain perfect numerical stability.}
\label{tab:bk_triton_stability}
\begin{tabular}{lrr}
\toprule
Implementation & NaN Occurrences & Rate \\
\midrule
PyTorch (vmap) & 0/100 & 0.0\% \\
Triton Kernel & 0/100 & 0.0\% \\
\midrule
\textbf{Target} & -- & \textbf{0.0\%} \\
\textbf{Achievement} & -- & \textbf{✓ PASS} \\
\bottomrule
\end{tabular}
\end{table}

Both implementations demonstrated perfect numerical stability with zero NaN occurrences across all trials, meeting the design requirement of 0\% NaN rate.

\subsubsection{Unit Test Results}

We implemented a comprehensive unit test suite covering numerical correctness, edge cases, complex arithmetic, gradient computation, and output properties. Table~\ref{tab:bk_triton_tests} presents the test results.

\begin{table}[ht]
\centering
\caption{Unit test results for BK-Core Triton implementation. 16 out of 17 tests passed (94.1\% pass rate).}
\label{tab:bk_triton_tests}
\begin{tabular}{lc}
\toprule
Test Category & Result \\
\midrule
Numerical Correctness (5 tests) & 5/5 ✓ \\
Edge Cases (4 tests) & 4/4 ✓ \\
Complex Arithmetic (2 tests) & 2/2 ✓ \\
Gradient Computation (2 tests) & 2/2 ✓ \\
Output Properties (3 tests) & 2/3 ✓ \\
Triton Availability (1 test) & 1/1 ✓ \\
\midrule
\textbf{Total} & \textbf{16/17 (94.1\%)} \\
\bottomrule
\end{tabular}
\end{table}

The test suite achieved 94.1\% pass rate (16/17 tests). The single failure was in device property checking, where the test expected \texttt{device='cuda'} but received \texttt{device='cuda:0'}. This is a trivial assertion issue rather than a functional problem, as both device specifications are semantically equivalent in PyTorch.

\subsubsection{Key Performance Indicators Summary}

Table~\ref{tab:bk_triton_kpi} summarizes the achievement of all design KPIs for the BK-Core Triton implementation.

\begin{table}[ht]
\centering
\caption{BK-Core Triton KPI achievement summary. All critical performance and correctness targets exceeded.}
\label{tab:bk_triton_kpi}
\begin{tabular}{lrrr}
\toprule
KPI Metric & Target & Achieved & Status \\
\midrule
Forward/Backward Speedup & $\geq 3.0\times$ & $189.94\times$ & ✓ PASS \\
MSE Error & $\leq 10^{-6}$ & $2.46 \times 10^{-10}$ & ✓ PASS \\
NaN Occurrence Rate & $0\%$ & $0\%$ & ✓ PASS \\
Test Pass Rate & $\geq 90\%$ & $94.1\%$ & ✓ PASS \\
\bottomrule
\end{tabular}
\end{table}
