...
\subsection{Memory Efficiency Evaluation}

We evaluated the memory efficiency of our proposed architecture on NVIDIA RTX 3080 (10GB VRAM). The experimental configuration consisted of vocabulary size 10,000, model dimension 512, 6 layers, sequence length 512, and batch size 2 for training.

\subsubsection{Parameter Compression}

Table~\ref{tab:param_compression} presents the measured parameter counts for each component.

\begin{table}[h]
\centering
\caption{Parameter compression results}
...
The HTT Embedding with rank=4 achieved 99.6\% compression (5.12M $\rightarrow$ 18.40K parameters). The AR-SSM layers with max\_rank=8 reduced transformer parameters by 97.1\%. Overall, the model size decreased from 29.16M to 616.09K parameters, representing a 97.9\% reduction.

\subsubsection{VRAM Usage During Training}

Table~\ref{tab:vram_training} shows the measured peak VRAM consumption during training with mixed precision (FP16).

\begin{table}[h]
\centering
\caption{VRAM usage during training (FP16)}
...
The optimized model consumed 69.1 MB peak VRAM compared to 456.3 MB for the baseline, achieving 84.8\% reduction. Parameter memory decreased by 84.6\% and activation memory by 84.9\%.
...
\begin{itemize}
    \item \textbf{Inference latency}: 1.5-2$\times$ increase due to gradient checkpointing
    ...
    \item \textbf{Convergence}: 10-15\% additional training steps required
\end{itemize}

\subsubsection{Scalability Analysis}

Extrapolating the measured compression ratios to a larger configuration (vocab=50K, d=1024, layers=12, seq\_len=2048), the baseline model would require approximately 8.2 GB VRAM, while the optimized version would require approximately 1.2 GB, fitting within typical GPU memory constraints.
