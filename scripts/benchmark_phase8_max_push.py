#!/usr/bin/env python3
"""
Phase 8 Maximum Push Benchmark
8GB VRAMで可能な限り大きなモデルを探索
"""
...
def test_config(config: ModelConfig) -> dict:
    ...
    print(f"\nTesting: d={config.d_model}, L={config.n_layers}, seq={config.seq_len}")
    ...
    try:
        ...
        print(f"  OK {result['parameters_billions']:.2f}B params, {result['peak_memory_gb']:.2f}GB VRAM")
...
def main():
    print("="*60)
    print("Phase 8 Maximum Push Benchmark")
    print("="*60)
    ...
    # d_model=2048で層数を増やす
    for n_layers in [24, 32, 40, 48, 56, 64]:
        configs.append(ModelConfig(
            d_model=2048, n_layers=n_layers, num_heads=16,
            seq_len=512, batch_size=1
        ))
    
    # d_model=2560で試す
    for n_layers in [24, 32, 40, 48]:
        configs.append(ModelConfig(
            d_model=2560, n_layers=n_layers, num_heads=20,
            seq_len=512, batch_size=1
        ))
    
    # d_model=3072で試す
    for n_layers in [24, 32, 40]:
        configs.append(ModelConfig(
            d_model=3072, n_layers=n_layers, num_heads=24,
            seq_len=512, batch_size=1
        ))
    
    # d_model=4096で試す（Phase 7相当）
    for n_layers in [16, 24, 32]:
        configs.append(ModelConfig(
            d_model=4096, n_layers=n_layers, num_heads=32,
            seq_len=512, batch_size=1
        ))
    ...
    # 結果保存
    output_file = Path('results/benchmarks/PHASE8_MAX_PUSH_BENCHMARK.json')
    ...
    if successful:
        ...
        print(f"   Config: d_model={max_config['d_model']}, n_layers={max_config['n_layers']}")
